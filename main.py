...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "   python app.py target.com --ports top\n"
            "\n"
            "2) Recon standar (umumnya cukup untuk baseline)\n"
            "   python app.py target.com --full --export html\n"
            "\n"
            "3) Deep preset (lebih agresif, lebih lama)\n"
            "   python app.py target.com --deep --export html\n"
            "\n"
            "4) Web scan dari port yang terbuka (mis. 8080/8443)\n"
            "   python app.py target.com --full --web --web-from-ports --export html\n"
            "\n"
            "5) Subdomain enumeration + resolve\n"
            "   python app.py target.com --subdomains --sub-resolve 200 --sub-brute --export html\n"
            "\n"
            "6) Port custom + service probe (fingerprint)\n"
            "   python app.py target.com --ports 80,443,8080,3389 --service-probe --export html\n"
            "\n"
            "7) Crawl + sitemap seeds (endpoint discovery)\n"
            "   python app.py target.com --web --crawl --sitemap --export html\n"
            "\n"
            "8) Authenticated scan (gunakan hanya jika punya izin)\n"
            "   python app.py target.com --full --cookie \"session=...\" --header \"Authorization: Bearer ...\" --export html\n"
        ),
    )
